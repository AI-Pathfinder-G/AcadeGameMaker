...
2026-08-24 갱신 결과, P0와 이동·플랫폼·미술 계약 반영은 끝났지만 **P1 장면 흐름과 새 AimArc 수치, 독립 검토가 남아 Unity 프로젝트 생성·에셋 임포트·구현은 아직 시작할 수 없다.**

## 핵심 상태

- gameplay 구현 가능한 `Approved` 스펙: 0개 (`VD-11` pre-Unity QA infrastructure만 Verified)
- 열린 P0 미결정: 0개
- 열린 P1 결정: 2개 (`OD-SCENE-001`, 새 `OD-AIMARC-001`)
...
Sol이 `OD-SCENE-001`과 새 `OD-AIMARC-001`을 통합 결정하고 소비 스펙을 Luna가 독립 검토한 뒤 관련 스펙을 `Approved`로 승격한다. 그 다음에만 Unity 프로젝트를 생성한다.
