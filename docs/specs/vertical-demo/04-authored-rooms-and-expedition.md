...
유효한 4방 원정 계획, 현재 방·소켓 계획, 방별 최대 필수 gap·벽 높이를 포함한 이동 검증 메타데이터, `RoomPlanSnapshot` 증적 또는 계약 오류를 낸다.

### Owned state

방 풀, 지원 snapshot, 현재 방 계획과 socket plan, 방별 최대 필수 gap·벽 높이와 기본 이동 검증 결과를 소유한다. 플레이어 물리·적 체력·전이 상태는 소유하지 않는다.
...
- 각 방 메타데이터는 최대 필수 gap과 최대 필수 벽 높이를 기록하고, 그 경로는 run 8.0·기준 점프 3.25u/28틱·대시 5.0u/15틱·벽차기 기준으로 완주 증적을 가져야 한다. 가벼운 상태 전용 경로는 선택적 shortcut일 뿐 필수 경로가 아니다.
...
- **REQ-ROOM-007:** 모든 방은 최대 필수 gap과 벽 높이를 메타데이터에 기록하고, 전이·선택 기술 없이 승인된 기준 이동 세트로 필수 경로를 통과했다는 검증 증적을 가져야 한다.
...
### AC-ROOM-006 — 기준 이동 방 도달성

- **Given** 네 지원 RoomPlanSnapshot과 각 방의 최대 필수 gap·벽 높이 메타데이터가 있고
- **When** 전이·선택 기술 없이 승인된 기준 이동 세트로 각 snapshot을 3회씩 완주하면
- **Then** 총 12회 모두 필수 경로를 통과하고 기록된 gap·벽 높이보다 큰 필수 지형이 없으며, 가벼운 상태 전용 경로는 shortcut으로만 남는다.

## Verification

방 메타데이터 구조 테스트, canonical JSON·SHA-256 snapshot 테스트, 자동 순환 저장 실패 테스트와 네 snapshot×3회 기준 이동 수동 완주 증적을 사용한다. 입출구 좌표와 저작 지오메트리는 VD-01의 승인된 수치·허용오차에 맞춰 검수한다.
