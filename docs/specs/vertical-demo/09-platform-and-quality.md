...
- **REQ-PLAT-002:** Windows 10/11 x64에서 키보드와 XInput 게임패드를 지원하고 마우스를 UI·조준에 허용하며, 16:9 2560×1440 출력 기준의 창/전체 화면 전환, 60 FPS, 1/60초 고정 물리 간격을 사용한다.
...
- **Then** 엔진 버전, 렌더 파이프라인, Windows x64 대상, 입력 기기, 2560×1440 출력 기준, 화면 모드, 60 FPS와 1/60초 물리 간격이 이 스펙과 일치한다.
