...
상태 머신 단위 테스트, 중복 실패 PlayMode 테스트, 실패·성공·비정상 종료 전후 상태 스냅샷과 원자 저장 실패 테스트를 사용한다. `profile.json` 필드·경로와 원자 저장 순서는 VD-09에서 고정하며 시작 시 손상 복구 순서는 `OD-PLAT-001`에서 고정한다.
