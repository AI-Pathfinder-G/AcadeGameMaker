...
- 판정: **환경 준비만 조건부 진행 가능 / Unity 프로젝트 생성·임포트·구현 보류**
...
| 저장소 | clean, 공개 저장소, `main` 단일 브랜치 | 부분 준비 |
| 브랜치 보호 | 없음 | 보강 필요 |
| Unity용 `.gitignore`/`.gitattributes` | 루트에 없음 | 생성 전 필수 |
...
| D-INPUT | 수직 데모 입력 | 키보드와 XInput 게임패드 모두 지원, 마우스는 UI만 |
| D-DISPLAY | 화면 기준 | 16:9, 1920×1080 출력 기준, 창 모드/전체 화면 전환 |
...
### 스펙 승인 전에 해결해야 하는 창작·게임플레이 결정

P0 여섯 항목은 수직 데모 구현을 직접 막고 있다.
...
P1 네 항목은 P0와 동시에 정리한다.

- `OD-MOV-001` — 이동 수치와 판정 허용치
- `OD-PLAT-001` — 입력·해상도·창 모드·FPS·저장 형식
...
1. 사용자가 A1, A3와 즉시 제품 기준을 승인한다.
2. 저장소 안전장치를 적용하고 기준 태그를 만든다.
3. Unity Hub와 6.3 LTS를 설치하되 프로젝트는 아직 만들지 않는다.
4. Sol이 P0 기본안을 통합 제출하고 사용자가 창작 핵심을 결정한다.
5. 관련 스펙을 Review에서 Approved로 승격한다.
